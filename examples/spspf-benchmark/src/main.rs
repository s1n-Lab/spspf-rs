...
use spspf_graphics::{Drawable, canvas::Canvas, shapes::Shape, colors::Colors};
...
fn psp_main() {
    ...
    let mut rects: Vec<(Shape, Vec2<f32>)> = Vec::new();
    ...
    while running {
        ...
        for rect_id in 0..rects.len() {
            match rects[rect_id].0 {
                Shape::Rect { vertices:_, indices:_, position, size, color:_ } => {
